...
| `src/guidance*.js`、`src/catalogs.js`、`src/tool-response-field-catalog.js`、`src/tool-option-catalog.js`、`src/diagnostic-catalogs.js`、`src/font-identity-basis-catalog.js`、`src/output-structure-catalog.js`、`src/unsupported-file-catalog.js`、`src/directory-handling-catalog.js` | 面向 AI agent 的机器可读指南、字段目录、警告目录、选项目录、忽略文件分类目录、目录处理目录和示例。`src/catalogs.js` 保留公共目录聚合与 re-export，体量较大的响应字段目录、选项目录、诊断目录、identity basis 目录、输出结构目录、忽略文件分类目录和目录处理目录放在独立文件。 |
