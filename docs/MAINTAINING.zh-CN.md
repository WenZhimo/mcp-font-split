...
| 维护者结构、验证和切片顺序 | 本文档 |
| 后续结构问题分类和 backlog | `docs/STRUCTURE-ACTION-PLAN.zh-CN.md` |
...
| `src/warning-code-catalog.js`、`src/error-response-catalog.js`、`src/diagnostic-catalogs.js` | warning code 和结构化错误响应事实。`warning-code-catalog.js` 只维护 `warningCodeCatalog` 条目；`error-response-catalog.js` 只维护 `errorResponseCatalog` 条目；`diagnostic-catalogs.js` 保持轻量 re-export，避免既有导入路径漂移。 |
