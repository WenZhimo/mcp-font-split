...
| `src/guidance*.js`、`src/local-verification-guidance.js`、`src/directory-workflow-guidance.js`、`src/configuration-recipes-guidance.js`、`src/catalogs.js`、`src/font-format-catalog.js`、`src/guidance-section-catalog.js`、`src/tool-response-field-catalog.js`、`src/tool-option-catalog.js`、`src/tool-option-enum-catalog.js`、`src/workflow-preset-catalog.js`、`src/diagnostic-catalogs.js`、`src/font-identity-basis-catalog.js`、`src/output-structure-catalog.js`、`src/unsupported-file-catalog.js`、`src/directory-handling-catalog.js` | 面向 AI agent 的机器可读指南、本地验证指南、目录工作流路由指南、配置 recipes、支持字体格式事实、guidance section 目录、字段目录、警告目录、选项目录、选项枚举事实、工作流 preset 目录、忽略文件分类目录、目录处理目录和示例。`src/agent-guidance.js` 保留最终 `get_agent_guidance` 组装层；`src/local-verification-guidance.js` 负责维护检查清单和真实语料输出解释；`src/directory-workflow-guidance.js` 负责源目录布局路由矩阵和示例；`src/configuration-recipes-guidance.js` 负责 `configurationRecipes`。`src/catalogs.js` 保留公共目录聚合与 re-export，体量较大的字体格式目录、guidance section 目录、响应字段目录、选项目录、选项枚举目录、工作流 preset 目录、诊断目录、identity basis 目录、输出结构目录、忽略文件分类目录和目录处理目录放在独立文件。 |
