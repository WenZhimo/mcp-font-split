...
| `src/font-split.js` | 公共运行时入口，导出 `splitFont`、`splitFontBatch`、`inspectFontInputs`、`organizeFontDirectory`，同时重新导出 guidance/runtime/status。它仍偏重，后续重构应继续拆薄。 |
...
| `src/input-*.js` | 输入扫描、输入目录判断、忽略文件摘要和目录结构预检。 |
