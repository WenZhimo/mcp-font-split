...
| `src/guidance*.js`, `src/catalogs.js`, `src/tool-response-field-catalog.js`, `src/tool-option-catalog.js`, `src/diagnostic-catalogs.js`, `src/font-identity-basis-catalog.js`, `src/output-structure-catalog.js`, `src/unsupported-file-catalog.js`, `src/directory-handling-catalog.js` | Machine-readable guidance, field catalogs, warning catalogs, option catalogs, unsupported-file category catalogs, directory handling catalogs, and agent examples. `src/catalogs.js` remains the public catalog aggregation and re-export layer; the large response-field, option, diagnostic, identity-basis, output-structure, unsupported-file, and directory-handling catalogs live in dedicated files. |
