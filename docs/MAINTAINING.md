...
| `src/guidance*.js`, `src/local-verification-guidance.js`, `src/directory-workflow-guidance.js`, `src/configuration-recipes-guidance.js`, `src/catalogs.js`, `src/font-format-catalog.js`, `src/guidance-section-catalog.js`, `src/tool-response-field-catalog.js`, `src/tool-option-catalog.js`, `src/tool-option-enum-catalog.js`, `src/workflow-preset-catalog.js`, `src/diagnostic-catalogs.js`, `src/font-identity-basis-catalog.js`, `src/output-structure-catalog.js`, `src/unsupported-file-catalog.js`, `src/directory-handling-catalog.js` | Machine-readable guidance, local verification guidance, directory workflow routing guidance, configuration recipes, supported font format facts, guidance section catalogs, field catalogs, warning catalogs, option catalogs, option enum facts, workflow preset catalogs, unsupported-file category catalogs, directory handling catalogs, and agent examples. `src/agent-guidance.js` remains the final `get_agent_guidance` assembly layer; `src/local-verification-guidance.js` owns maintenance checklists and real-corpus output interpretation; `src/directory-workflow-guidance.js` owns source-layout routing matrices and examples; `src/configuration-recipes-guidance.js` owns `configurationRecipes`. `src/catalogs.js` remains the public catalog aggregation and re-export layer; the large font-format, guidance-section, response-field, option, option-enum, workflow-preset, diagnostic, identity-basis, output-structure, unsupported-file, and directory-handling catalogs live in dedicated files. |
