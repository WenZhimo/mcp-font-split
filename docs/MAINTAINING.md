...
| `src/guidance*.js`, `src/catalogs.js`, `src/tool-response-field-catalog.js`, `src/tool-option-catalog.js`, `src/diagnostic-catalogs.js`, `src/font-identity-basis-catalog.js` | Machine-readable guidance, field catalogs, warning catalogs, option catalogs, and agent examples. `src/catalogs.js` remains the public catalog aggregation and re-export layer; the large response-field, option, diagnostic, and identity-basis catalogs live in dedicated files. |
