...
| `src/guidance*.js`, `src/catalogs.js`, `src/tool-response-field-catalog.js`, `src/tool-option-catalog.js` | Machine-readable guidance, field catalogs, warning catalogs, option catalogs, and agent examples. `src/catalogs.js` remains the public catalog aggregation and re-export layer; the large response-field and option catalogs live in dedicated files. |
