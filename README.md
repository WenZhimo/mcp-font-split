...
`localVerificationOutputGuide.completionReportGuide` 是给 agent 的阶段总结检查表。门禁通过后，可用其中的 `requiredClaims[]` 和 `conciseReportTemplate[]` 汇报 compact 检查、代表性真实语料状态、全库根扫描支持字体数、忽略文件覆盖、功能覆盖和代表性输出审计；同时必须遵守 `forbiddenClaims[]`，不要声称每个字体或每个目录都已人工验收，也不要暗示压缩包被解压验证。
