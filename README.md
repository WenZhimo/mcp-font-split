...
- `layoutDecision`：顶层紧凑路线摘要，汇总 `layoutKind`、推荐分组、主线路由、源安全信号、原目录安全预览状态和 copy-only 暂存状态。它适合 agent 先快速判断“下一步看哪里”，但不是整理或拆分已经成功的证明。
...
- `directoryWorkflowSummary.planVisibility`：说明本次响应是否包含详细 `plan[]`。当 `includePlan: false` 时，`plan[]` 会被省略，但 `planActionSummary`、`layoutDecision`、`organizationDecision`、`sourceLayoutMismatchSummary`、`recommendedNextActions[]`、`organizationWarnings[]`、`layout`、`safetySummary` 和 `batchPolicySummary` 仍可用于大目录 triage；如果写入前需要确认每个文件的目标路径，应按其中的 `rerunWithPlanArgs` 重新 dry-run。
...
应把 `layoutDecision`、`organizationDecision`、`directoryWorkflowSummary`、`sourceLayoutMismatchSummary` 和 `sourceLayoutMismatchSummary.decisionChecklist` 当作主线提示，而不是成功证明。它们帮助 agent 选择下一步分支；真正继续前仍要检查 `recommendedNextActions[]`、`organizationWarnings[]`、`planActionSummary`、`directoryWorkflowSummary.planVisibility` 和可用时的 `plan[]`。
