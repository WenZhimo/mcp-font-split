...
- `directoryWorkflowSummary`：本次响应里的目录工作流导航摘要，用来串起布局复核、安全批量预览、可选 copy-only 暂存、reviewed 批量写入和必须执行的输出审计。它会重复源目录安全信号、路线选择、`workflowSteps[]`、成功标准和非直觉行为提示。
...
应把 `organizationDecision` 和 `directoryWorkflowSummary` 当作主线提示，而不是成功证明。它们帮助 agent 选择下一步分支；真正继续前仍要检查 `recommendedNextActions[]`、`organizationWarnings[]`、`planActionSummary` 和可用时的 `plan[]`。
