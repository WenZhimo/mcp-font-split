...
需要给 agent 或脚本稳定解析时，使用 `--json` 或 `FONT_SPLIT_JSON=true`。JSON 模式不会输出进度条或人类摘要；成功时 stdout 是 `{ ok: true, runner, options, result }`，失败时 stdout 是 `{ ok: false, runner, options, name, error, details? }` 且进程退出码仍为非零。
