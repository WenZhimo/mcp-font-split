...
> **项目状态：仍在完善中，尚未正式发布。**
>
> 当前接口、默认参数、输出字段、目录整理策略和文档说明都可能随时调整。使用或集成时请以当前仓库代码、实时 MCP schema、`get_agent_guidance` 返回值和 API 文档为准。
...
`batch:run` 是给 agent 和维护者使用的安全批量入口。默认真实运行走 `reviewed-write`；`--dry-run` 或 `FONT_SPLIT_DRY_RUN=true` 会走 `safe-preview`。`FONT_SPLIT_WORKFLOW_PRESET` 里的 `default` 不是有效值；无效 preset 拒绝、无效环境变量或无效位置数字都会以 `BatchRunConfigurationError` 失败，并带 `errorType`、允许值或期望类型。枚举型、布尔型或数字型配置如果填了无效值，会被拒绝而不是静默回退。需要稳定 JSON 时用 `--json` 或 `FONT_SPLIT_JSON=true`。
