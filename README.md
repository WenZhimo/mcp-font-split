...
- `get_agent_guidance.nextToolDecisionSummary` 是更短的“下一步该调用哪个工具”索引：它会按 setup、输入预检、目录结构判断、copy-only 暂存、批量预览、审查后写入和输出审计给出首选工具与模板 ID。其 `quickStartCallExamples[]` 由 `safeInvocationTemplates[]` 派生，用于给出最常见路线的最小调用参数；它只用于快速路由，不能替代实际工具响应字段和 `successCriteria`。
