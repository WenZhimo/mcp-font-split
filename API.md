...
`directoryOrganizationQuickAnswer` is the compact answer for source-layout mismatch questions. It says the helper tool is `organize_font_directory`, the first call should be `workflowPreset: "safe-preview"`, reviewed writes are copy-only into `outputDir`, source fonts are never moved/deleted/rewritten, and organized `outputDir` is source-like staging rather than final split output. Use it before the longer `directoryWorkflowDecisionMatrix[]` when the user simply asks whether directory organization exists or whether it is destructive.

`toolSafetyQuickReference` is the compact per-tool safety table for all public tools. Each entry summarizes `defaultWritesFiles`, `sourceDestructive`, `sourceFilesMovedDeletedOrRewritten`, write scope, source backup expectations, safe-preview arguments when relevant, and `mustInspectFields`. Use it before choosing a tool or answering write-safety questions; after a write-capable tool runs, still inspect the actual `sourceSafetyDecision`, `safetySummary`, `outputStructureDecision`, and related audit fields.
...
`verificationChecklist[]` also includes local maintenance gates for agents maintaining this package:

- `local-compact-check-passed` points to `npm run check:compact` and `npm run --silent check:compact -- --json`. It is the low-noise wrapper around the standard syntax/smoke checks and returns `compact-check-result` with failed-step stdout/stderr tails when a child gate fails.
...
- `localVerificationOutputGuide` is the machine-readable companion for interpreting these local command outputs. It names `standardCommand` for the compact check and `reliabilityGateDecision` as the real-corpus primary decision field, then lists required output fields, pass criteria, status meanings, and non-intuitive scope warnings.
- The suite output prints a short `real-corpus suite summary` and repeats it in JSON `humanSummary`; use it to avoid confusing fixed target counts with the full corpus scan.
- The JSON also includes top-level `reliabilityGateDecision` and `corpusCountGuide`. Check `status`, `reliabilityGatePassed`, `blockingReasonCodes`, `fullCorpusFontCountField`, and `targetCountsAreFullCorpusCounts` before interpreting the run, then use `corpusCountGuide.fullCorpus` and `corpusCountGuide.representativeTargets` for the shortest machine-readable count explanation.
- `status: "pass"` means the representative feature chain passed, not that every font directory was manually accepted.
- `testScope` separates `corpusScan` as the full bounded root scan, `targetSampling` as representative dry-run sampling, and `representativeWriteAudit` as one bounded real write plus output audit path.
- Compact `coverageSummary.functionalCoverage[]` keeps feature IDs and pass/fail status while omitting large evidence. `coverageSummary.toolCoverageSummary` maps those feature paths to public MCP tools; `allRequiredToolsCovered: true` means `get_agent_guidance`, `get_runtime_status`, `inspect_font_inputs`, `organize_font_directory`, `split_font`, `split_font_batch`, and `inspect_split_output` were all covered through representative real-corpus paths.
