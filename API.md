...
`nextToolDecisionSummary` is a compact routing index for agents asking "which tool should I call next?". It maps common situations to the first tool and, when available, a `safeInvocationTemplates[]` template ID. It is intentionally shorter than `recommendedWorkflowPlan`; use it to choose the route, then inspect the referenced template or real tool response fields and satisfy `successCriteria`.
