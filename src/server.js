...
server.registerTool(
  'get_agent_guidance',
  {
    ...
    description: 'Call this first when an AI coding assistant needs to choose a safe font-splitting workflow. It returns workspace path rules, directoryWorkflowDecisionMatrix, directoryWorkflowExamples, safeInvocationTemplates, warningCodeCatalog, toolResponseFieldCatalog, recommended tool order, defaults, and response fields to inspect. Use detailLevel or sections for a compact or focused response.',
    inputSchema: {
...
server.registerTool(
  'split_font_batch',
  {
    ...
    description: 'Call this when the user wants to split many local font files under a directory. dryRun defaults to false and can write output files; agents should set dryRun:true with includeResults:true to preview scan, dedupe, naming, skip decisions, safetySummary, outputTreeInsideInputTree, and batchWarnings before writing. Source font files are never moved or deleted, but writesSourceTree can be true when outputRoot is inside or equal to inputDir.',
    inputSchema: {
...
server.registerTool(
  'inspect_split_output',
  {
    ...
    description: 'Call this to summarize and structurally inspect generated cn-font-split output files in an output directory. Inspect structureSummary, maxFilesHit, and inspectionWarnings before treating an audit as complete; use includeFiles:false and includeFamilies:false for compact large-output summaries.',
    inputSchema: {
