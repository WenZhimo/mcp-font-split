...
server.registerTool(
  'organize_font_directory',
  {
    ...
    description: 'Call this when the source font directory layout does not match the desired batch grouping. It defaults to dryRun true and is source-non-destructive: it never moves or deletes source files, and when dryRun is false it only copies selected fonts into outputDir. Inspect sourceSafetyDecision, layoutDecision.directoryHandling, stagingDirectoryDecision, safetySummary, batchPolicySummary, directoryWorkflowSummary, sourceLayoutMismatchSummary, sourceLayoutMismatchSummary.decisionChecklist, recommendedBatchPreviewArgs, and outputTreeInsideInputTree because writesSourceTree can be true when outputDir is inside or equal to inputDir. recommendedBatchPreviewArgs preserves the current maxFiles scan cap for copyable follow-up previews. stagingDirectoryDecision explicitly marks outputDir as source-like staging, not split output. For flat/nested/mixed/output-inside-input routing, get_agent_guidance sections:["examples"] includes source-layout-mismatch-comparison.',
    inputSchema: {
      inputDir: z.string().optional().describe('Directory to scan, relative to the font workspace. Defaults to the workspace root.'),
