...
if (scenario === 'single') {
...
} else if (scenario === 'real-corpus-write') {
  const corpusRoot = path.resolve(process.argv[3] || process.env.FONT_SPLIT_REAL_CORPUS_DIR || path.join(process.cwd(), '..'));
  const requestedInputDir = process.argv[4] || null;
  const outputRoot = process.argv[5] || 'font-split-mcp/.font-split-real-corpus-write-output';
  const maxFiles = Number.parseInt(process.argv[6] || process.env.FONT_SPLIT_REAL_CORPUS_MAX_FILES || '50000', 10);
  const limit = Number.parseInt(process.argv[7] || process.env.FONT_SPLIT_REAL_CORPUS_WRITE_LIMIT || '20', 10);
  if (!Number.isFinite(maxFiles) || maxFiles < 1 || !Number.isFinite(limit) || limit < 1) {
    throw new Error('Expected maxFiles and limit to be positive integers for real-corpus-write smoke.');
  }
  process.env.FONT_SPLIT_ROOT = corpusRoot;

  const resolvedOutputRoot = path.resolve(corpusRoot, outputRoot);
  if (!isInsidePath(corpusRoot, resolvedOutputRoot) || !path.basename(resolvedOutputRoot).startsWith('.font-split-')) {
    throw new Error('real-corpus-write only clears and writes a generated .font-split-* output directory inside the corpus root.');
  }
  await fs.rm(resolvedOutputRoot, { recursive: true, force: true });
  ...
  if (corpusInspection.supportedFontCount < 1 || corpusInspection.filesIncluded !== false) {
    throw new Error('Expected real corpus write smoke to first inspect the full bounded corpus root.');
  }

  const sample = await findRealCorpusSample({ corpusRoot, requestedInputDir, maxFiles });
  const organization = await organizeFontDirectory({
    inputDir: sample.inputDir,
    outputDir: 'font-split-mcp/.font-split-real-corpus-write-organized-preview',
    workflowPreset: 'structure-first',
    maxFiles,
  });
  if (organization.dryRun !== true || organization.writesOutputTree !== false || organization.recommendedBatchPreviewArgs?.inputDir !== sample.inputDir) {
    throw new Error('Expected real corpus write smoke to get no-write organization preview args before writing batch output.');
  }

  console.log('Real corpus write smoke:', corpusRoot, 'sample:', sample.inputDir, '->', outputRoot, 'limit:', limit, 'maxFiles:', maxFiles);
  const batchWrite = await splitFontBatch({
    inputDir: sample.inputDir,
    outputRoot,
    workflowPreset: 'reviewed-write',
  ...
  if (
    ...
    || batchWrite.errorCount !== 0
    || auditAction?.tool !== 'inspect_split_output'
    || auditAction?.suggestedArgs?.outDir !== outputRoot
  ) {
    throw new Error('Expected real corpus write smoke to write batch output and recommend an output audit.');
  }

  const audit = await inspectSplitOutput(auditAction.suggestedArgs);
  const actionRequiredInspectionWarnings = (audit.inspectionWarnings || [])
    .filter((warning) => !['output-files-omitted', 'output-families-omitted'].includes(warning.code));
  if (
    audit.maxFilesHit !== false
    || audit.structureSummary?.conforms !== true
    || actionRequiredInspectionWarnings.length > 0
    || audit.fontEntryCount < 1
  ) {
    throw new Error('Expected real corpus write output audit to conform without action-required inspection warnings.');
  }
  assertInspectFieldsExist(auditAction, {
    inspect_split_output: audit,
  }, 'real-corpus-write audit action');
  ...
  console.log(JSON.stringify({
    ...
    batchWrite: {
      ...
      processedFontCount: batchWrite.processedFontCount,
      errorCount: batchWrite.errorCount,
      batchWarnings: batchWrite.batchWarnings,
      processingSummary: batchWrite.processingSummary,
      recommendedNextActions: batchWrite.recommendedNextActions,
    },
    audit: {
      ...
      fontEntryCount: audit.fontEntryCount,
      manifestCount: audit.manifestCount,
      subsetOutputCount: audit.subsetOutputCount,
      singleWoff2OutputCount: audit.singleWoff2OutputCount,
      copyOriginalOutputCount: audit.copyOriginalOutputCount,
      inspectionWarnings: audit.inspectionWarnings,
