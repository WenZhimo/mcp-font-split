...
Use `--json` or `FONT_SPLIT_JSON=true` when an agent or script needs stable parsing. JSON mode suppresses progress and human summaries; stdout is `{ ok: true, runner, options, result }` on success, or `{ ok: false, runner, options, name, error, details? }` on failure while the process still exits nonzero.
