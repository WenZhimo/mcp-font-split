...
- For AI coding assistants, call `get_agent_guidance` first when the workflow is unclear. It returns compact guidance by default: recommended tool order, default policies, path rules, `toolOptionCatalog`, response fields, a verification checklist, `errorResponseCatalog`, and `localVerificationOutputGuide` for interpreting local real-corpus smoke output before claiming success. `guidanceView` reports which sections were included or omitted.
...
- `recommendedBatchPreviewArgs`: copyable no-write `split_font_batch` preview arguments
