...
- `get_agent_guidance` returns `configurationRecipes[]`, which map common intent to preset-first arguments, such as preserving every source font, grouping by source folders, grouping by font metadata, fast structure scans, copy-only staging, or large reviewed writes. Recipes are safe starting points; agents still need to run the preview/write tools and inspect the listed response fields.
...
- `get_agent_guidance` includes `recommendedWorkflowPlan`, an ordered route map for the selected `workflow`. It references the safe template IDs for phases such as preflight, layout decision, preview, reviewed write, and output audit; it does not replace checking the listed response fields.
