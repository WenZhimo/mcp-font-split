...
- For AI coding assistants, call `get_agent_guidance` first when the workflow is unclear. It returns compact guidance by default: recommended tool order, default policies, path rules, `toolSafetyQuickReference`, `toolOptionCatalog`, `outputStructureCatalog`, response fields, a verification checklist, `errorResponseCatalog`, and `localVerificationOutputGuide` for interpreting local real-corpus smoke output before claiming success. `guidanceView` reports which sections were included or omitted.
- `get_agent_guidance` returns `projectStatusNotice`, a machine-readable reminder that the package is still being refined and not formally released yet: interfaces, defaults, response fields, and directory-organization policy may change, so automation should trust the current repository code, live MCP schema, `get_agent_guidance`, and the current API docs instead of preserving unreleased compatibility cruft.
...
- `get_agent_guidance` returns `batchCustomizationQuickReference[]`, a shorter entrypoint than `batchPolicyGuide` for common batch customization requests. It maps intents such as preserving every source font, grouping by source folders or metadata, using plain names or source suffixes, and collecting errors to minimal `overrideArgs`, copyable `safe-preview` / `reviewed-write` args, required inspect fields, and non-intuitive behavior.
- `get_agent_guidance` returns `directoryOrganizationQuickAnswer`, a direct answer to whether the package has a directory organization helper and whether it can damage source directories. Use `organize_font_directory`; its default `workflowPreset: "safe-preview"` writes nothing, and reviewed `reviewed-write` calls are copy-only into `outputDir`. Source fonts are never moved, deleted, or rewritten, and `outputDir` is source-like staging rather than final split output.
- `get_agent_guidance` returns `toolSafetyQuickReference`, a compact per-tool table for all 7 public tools. It summarizes `defaultWritesFiles`, `sourceDestructive`, `sourceFilesMovedDeletedOrRewritten`, write scope, whether source backup is needed, and the response fields that must be inspected after real calls. Use it to answer which tools write files and whether any tool moves/deletes/rewrites source fonts; still verify `sourceSafetyDecision`, `safetySummary`, and output audits after write-capable tools run.
...
- `get_agent_guidance` returns `errorResponseCatalog`, which explains when MCP error text should be parsed as JSON and how to route `FontSplitConfigurationError` with `errorType: "configuration-error"`, `BatchSplitError` with `errorType: "batch-split-error"`, and plain unstructured errors. Invalid explicit configuration values are rejected instead of silently falling back; omit an option to use its default rather than passing an invalid enum, boolean, or numeric value.
