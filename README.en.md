...
- `get_agent_guidance.nextToolDecisionSummary` is the shorter "which tool should I call next?" index. It maps setup, source preflight, source-layout routing, copy-only staging, batch preview, reviewed write, and output audit to the first tool and template ID. It is only a routing shortcut; agents must still use `safeInvocationTemplates[]`, inspect real response fields, and satisfy `successCriteria`.
