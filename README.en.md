...
- `get_agent_guidance` returns `errorResponseCatalog`, which explains when MCP error text should be parsed as JSON and how to interpret `FontSplitConfigurationError` with `details.summaryType: "configuration-error"`, `BatchSplitError`, and plain unstructured errors.
