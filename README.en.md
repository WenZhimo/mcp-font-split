...
- `directoryWorkflowSummary`: response-local navigation summary for layout review, safe batch preview, optional copy-only staging, reviewed batch write, and required output audit. It repeats source-safety signals, route choice, `workflowSteps[]`, success criteria, and non-intuitive behavior notes.
...
Treat `organizationDecision` and `directoryWorkflowSummary` as route hints, not proof of success. They help agents choose the next branch; before continuing, still inspect `recommendedNextActions[]`, `organizationWarnings[]`, `planActionSummary`, and `plan[]` when available.
