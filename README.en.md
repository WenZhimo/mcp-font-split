...
- `get_agent_guidance` also returns `unsupportedFileCategoryCatalog`, which explains the representative extensions and handling behavior behind `unsupportedFileSummary.byCategory[]` categories such as `archive`, `document`, and `unsupported-font`; archives are reported but not extracted, copied, or split.
