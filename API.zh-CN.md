...
`nextToolDecisionSummary` 是面向“下一步该调用哪个工具？”的紧凑路由索引。它把常见情形映射到首选工具，并在可用时给出 `safeInvocationTemplates[]` 的模板 ID。其中的 `quickStartCallExamples[]` 会从安全模板生成常见路线的最小占位参数，例如输入预检、目录规划、copy-only 暂存、批量预览、审查后写入和输出审计。它比 `recommendedWorkflowPlan` 更短，只用于选择路线；继续前仍要查看引用的模板或实际工具响应字段，并满足 `successCriteria`。
