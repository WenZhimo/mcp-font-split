...
`nextToolDecisionSummary` 是面向“下一步该调用哪个工具？”的紧凑路由索引。它把常见情形映射到首选工具，并在可用时给出 `safeInvocationTemplates[]` 的模板 ID。它比 `recommendedWorkflowPlan` 更短，只用于选择路线；继续前仍要查看引用的模板或实际工具响应字段，并满足 `successCriteria`。
