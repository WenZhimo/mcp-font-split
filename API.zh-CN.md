...
`organizationDecision`、`directoryWorkflowSummary` 和 `sourceLayoutMismatchSummary` 是压缩路线提示，不是路线已经完成的证明。用它们选择下一步分支后，仍要检查 `recommendedNextActions[]`、`organizationWarnings[]`、`planActionSummary`、`directoryWorkflowSummary.planVisibility`，以及可用时的 `plan[]`。
